...
const TicketBooking = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Determine which airlines are actually used by tickets and
        // initialize filters only for those airlines so the UI shows
        // only airlines that exist in the ticket data.
        const usedAirlineIds = new Set(ticketsData.map((t) => t.airline).filter(Boolean));

        const initialAirlineFilters = {};
        Object.values(airlinesById).forEach((airline) => {
          if (usedAirlineIds.has(airline.id)) {
            initialAirlineFilters[airline.name] = false;
          }
        });

        // Build airlineMapData only for used airlines to ensure the card
        // shows proper names/logos for tickets
        const airlineMapDataFiltered = Object.values(airlinesById).reduce((map, airline) => {
          if (usedAirlineIds.has(airline.id)) {
            map[airline.id] = { name: airline.name, logo: airline.logo };
          }
          return map;
        ...
        setAirlineMap(airlineMapDataFiltered);
  ...
  const handleShowAll = () => {
    ...
    setDestination("");
    setTravelDate("");
    // Reset route filters only for routes currently present
    ...
    // Reset airline filters only for airlines currently present
