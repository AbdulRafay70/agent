  ...
  return (
    <div className="container-fluid p-3">
      ...
      {packageData.map((pkg, index) => {
        ...
        const commonPrices =
          (pkg.adault_visa_price || pkg.adault_visa_selling_price || 0) +
          (pkg.transport_price || pkg.transport_selling_price || 0) +
          (ticketInfo?.adult_price || 0) +
          (pkg.food_price || pkg.food_selling_price || 0) +
          (pkg.makkah_ziyarat_price || pkg.makkah_ziyarat_selling_price || 0) +
          (pkg.madinah_ziyarat_price || pkg.madinah_ziyarat_selling_price || 0);

        // Helper to sum hotel component per bed type
        const sumHotel = (field) => pkg?.hotel_details?.reduce((sum, h) => sum + (h[field] || 0) * (h.number_of_nights || 0), 0) || 0;

        // Calculate prices with Full Agency awareness
        const getFinalTotal = (calculated, backendPrice) => {
          if (backendPrice) return Number(backendPrice);
          const agencyData = localStorage.getItem('agencyData');
          const parsedAgency = agencyData ? JSON.parse(agencyData) : null;
          const agencyType = parsedAgency?.agency_type;
          const serviceCharge = (agencyType === 'Full Agency') ? 0 : 500;
          return calculated + serviceCharge;
        };

        const totalSharing = getFinalTotal(sumHotel('sharing_bed_price') + commonPrices, pkg.sharing_price);
        const totalQuint = getFinalTotal(sumHotel('quaint_bed_price') + commonPrices, pkg.quint_price);
        const totalQuad = getFinalTotal(sumHotel('quad_bed_price') + commonPrices, pkg.quad_price);
        const totalTriple = getFinalTotal(sumHotel('triple_bed_price') + commonPrices, pkg.triple_price);
        const totalDouble = getFinalTotal(sumHotel('double_bed_price') + commonPrices, pkg.double_price);

        // Map room types for the calculatePackagePrice function
        const priceMap = {
          'sharing': totalSharing,
          'quint': totalQuint,
          'quad': totalQuad,
          'triple': totalTriple,
          'double': totalDouble
        ...
        const infantHotels = pkg.infant_visa_selling_price || pkg.infant_visa_price || 0;
        const totalinfant = infantPrices + infantHotels;

        const childPrices = (pkg?.adault_visa_selling_price || pkg?.adault_visa_price || 0) - (pkg?.child_visa_selling_price || pkg?.child_visa_price || 0);
